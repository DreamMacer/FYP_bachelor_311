...
class QLearnerorigin:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        actions = batch["actions"][:, :-1]#是已经被选择的动作
        ...
        for t in range(batch.max_seq_length):
            curr_obs = batch["obs"][:,t]  # [batch_size, n_agents, obs_dim]
        ...
        obs_tensor = th.stack(obs_list, dim=1)
        ...
        for t in range(batch.max_seq_length):
            ...
            target_curr_obs = batch["obs"][:,t]
            target_obs_list.append(target_curr_obs)
        ...
        # obs_tensor形状: [batch_size, max_seq_length, n_agents, obs_dim]
        target_obs_tensor = th.stack(target_obs_list, dim=1)
        ...
        if self.mixer is not None:
            ...
            if self.args.mixer == "qmix":
                ...
                chosen_action_qvals = self.mixer(chosen_action_qvals, obs_tensor[:, :-1], batch["state"][:, :-1])
                target_max_qvals = self.target_mixer(target_max_qvals, target_obs_tensor[:, 1:], batch["state"][:, 1:])
