...
class EVParallelEnv(ParallelEnv):  #  ParallelEnv, EzPickle
    ...
    def __init__(
        ...
        self.death_tracker = np.zeros(self.n_agents)
        self.total_waiting_time =0.0
        self.step_waiting_time =0.0
        ...
        for ev_id in self.possible_agents:
            if self.connection.vehicle.getParameter(ev_id, "has.battery.device").lower() == "true":
                ...
                if current_capacity == -1:
                    current_capacity = random.uniform(0.3,0.6) * max_capacity
    ...
    def step(self, actions: Dict[str, int]):
        ...
        if all(self.terminations.values()) or all(self.truncations.values()):
            "清空智能体就会reset"
            self.agents = []
            return {},reward,self.terminations, self.truncations, self.infos
        if all([self.terminations[agent] or self.truncations[agent]for agent in self.agents]):
            self.agents = []
            return {},reward,self.terminations, self.truncations, self.infos
        
        return self.observations, reward, self.terminations, self.truncations, self.infos
    ...
    def _to_CS(self, agent, cs_id):
        ...
        self.connection.vehicle.setParkingAreaStop(
            ...
            # pos=station_end_pos,
            # laneIndex=0,
            duration= 199999.0  # 充电持续时间,不能太大，否则变成浮点数了
        )
    ...
    def _get_infos(self):
        ...
        max_time = self.n_agents * self.sim_max_time 
        # 计算奖励相关信息
        for ev_id, ev in self.vehicles.items():
            if (not self.truncations[ev_id]) and (not self.terminations[ev_id]) and (ev.status < self.num_stations):
                self.step_waiting_time += self.connection.vehicle.getWaitingTime(ev_id)
        self.cs_edges = {}
        
        for cs_id in self.cs_ids:
            lane = self.connection.chargingstation.getLaneID(cs_id)
            edge = self.connection.lane.getEdgeID(lane)
            self.cs_edges[cs_id] = edge
            self.wait_time += self.connection.lane.getWaitingTime(lane)

        
        self.total_waiting_time += self.step_waiting_time
        self.mean_waiting_time = self.wait_time/self.n_agents
        total_ratio = self.total_waiting_time/max_time
        vehicle_wait_time = self.wait_time
        ...
        self.infos.update(
            {
                ...
                "veh_waiting_time": vehicle_wait_time                             
            }
        )
        return self.infos
    ...
    def get_ev_reward(self):
        ...
        reward = 0
        delta_waiting_time = 0 #累计所有电车waiting time
        delta_choice_value = 0 #累计所有电车的选择
        delta_battery_safety = 0#累计所有电车的电池状态
        delta_reward =0#累计本回合的任务奖励，奖励是延迟的
        alive_num =0#记录运行中智能体个数
        "ev_id: str , ev: 实体"
        "只计算还本回合开始前还存活的智能体(不包括已完成任务和任务失败的)"
        for ev_id, ev in self.vehicles.items():
           ev_idex = self.EV.ev_dict[ev_id]
           if not self.death_tracker[ev_idex]:
               alive_num += 1
               current_capacity = float(self.connection.vehicle.getParameter(ev_id, "device.battery.chargeLevel"))
               max_capacity = float(self.connection.vehicle.getParameter(ev_id, "device.battery.capacity"))
               battery_ratio = current_capacity / max_capacity
               #本回合智能体死了
               if self.truncations[ev_id]:
                   self.death_tracker[ev_idex] = 1
                   delta_reward -= 1
                #本回合智能体完成任务
               elif self.terminations[ev_id]:
                   self.death_tracker[ev_idex] = 1
                   delta_reward += 1
                #在路上或者在充电
                #在路上
               elif ev.status < self.num_stations:
                    delta_waiting_time  += self.connection.vehicle.getWaitingTime(ev_id)
                    if battery_ratio < 0.3:
                        delta_battery_safety -= 1.0
                    else:
                        delta_battery_safety += 1.0
                    if ev._locked_cs is not None:
                        delta_choice_value += 1.0
                    if ev._missed_cs is not None:
                        delta_choice_value -= 1.0
                    if ev._failure_set:
                        ev._failure_set = False
                        delta_choice_value -= 1.0
                    if ev._is_rerouted and (not self.is_reset):
                        before_cs_id = self.EV.charging_stations_dict[ev._route_before]
                        cs_lane_before = self.connection.chargingstation.getLaneID(before_cs_id)
                        now_cs_id = self.EV.charging_stations_dict[ev._route_after]
                        ...
                        before_vehicle_density= ev.get_lane_density(cs_lane_before)
                        after_vehicle_number = self.connection.lane.getLastStepVehicleNumber(cs_lane_now)
                        after_vehicle_density = ev.get_lane_density(cs_lane_now)
                        ...
                        if relative_before_score < relative_after_score:
                                #选择不正确
                            ev._is_rerouted = False
                            ev._route_before = None
                            ev._route_after = None
                            delta_choice_value -= 1.0
                        else:
                                #选择正确
                            ev._is_rerouted = False
                        ...
                        congestion = []
                        for cs_id in ev.charge_station_IDs:
                            cs_lane_id = self.connection.chargingstation.getLaneID(cs_id)
                            congestion.append(ev.get_lane_density(cs_lane_id))
                        max_density = max(congestion)
                        min_density = min(congestion)               
                        logger.debug(f"FOR first choice or choice unchanged: status:{ev.status}")
                        now_cs_id = self.EV.charging_stations_dict[ev.status]
                        cs_lane_now = self.connection.chargingstation.getLaneID(now_cs_id)
                        now_density = ev.get_lane_density(cs_lane_now)
                        ...
                        if norm_density < 0.2:
                            delta_choice_value += 1.0
                #在充电
               elif self.num_stations <= ev.status < 2* self.num_stations:
                   delta_choice_value += 1.0
                   ev._is_stopped = False
        reward = 0.4* delta_reward + 0.15* delta_battery_safety/alive_num - 0.5 *delta_waiting_time/alive_num + 0.05*(delta_choice_value/alive_num)
        return reward
