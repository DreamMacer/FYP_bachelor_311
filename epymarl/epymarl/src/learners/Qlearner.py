...
from modules.mixers.tmix3 import DTQN
from modules.mixers.tmix_vanilla import TMixer
from modules.mixers.tmix2 import TMixer as tmixer
import torch as th
...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # print(f"rewards: {rewards.shape}, actions: {actions.shape}, terminated: {terminated.shape}, mask: {mask.shape}, avail_actions: {avail_actions.shape}")
        if self.args.standardise_rewards:
        ...
        # print(f"rewardsexpeand: {rewards.shape},common_reward: {self.args.common_reward}") 
        ...
        if self.mixer is not None:
            ...
            elif self.args.mixer == "dtqn":            
                chosen_action_qvals = self.mixer(chosen_action_qvals, mac_hidden_states[:,:-1], batch["state"][:, :-1])
                target_max_qvals = self.target_mixer(target_max_qvals, target_mac_hidden_states[:,1:], batch["state"][:, 1:])
            elif self.args.mixer == "fastmix":            
                chosen_action_qvals = self.mixer(chosen_action_qvals, mac_hidden_states[:,:-1], batch["state"][:, :-1])
                target_max_qvals = self.target_mixer(target_max_qvals, target_mac_hidden_states[:,1:], batch["state"][:, 1:])
            else :
                chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1])
        ...
        self.training_steps += 1#为了target更新方式
        if (
