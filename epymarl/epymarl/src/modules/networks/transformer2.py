...
class TransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        # 使用 register_buffer 管理掩码
        attn_mask = torch.triu(torch.ones(history_len, history_len), diagonal=1).fill_(float("-inf")).to(device)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 重塑回原始形状 [batch_size, n_agents, seq_len, embed_size]
        return x
...
class TransformerIdentityLayer(TransformerLayer):
    ...
    #     # print(f"x_norm1: {x_norm1.shape},x: {x.shape},x_norm1.size(1): {x_norm1.size(1)}")
    #     attention, self.alpha = self.attention(
    #         x_norm1,
    #         x_norm1,
    #         x_norm1,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x=x.to(self.device)
        x_norm1 = self.layernorm1(x)
        seq_len = x_norm1.size(1)

        # 动态生成或调整 attn_mask
        if self.attn_mask.size(0) < seq_len or self.attn_mask.size(1) < seq_len:
            new_mask = torch.zeros(seq_len, seq_len)
            new_mask[:self.attn_mask.size(0), :self.attn_mask.size(1)] = self.attn_mask
            self.attn_mask = new_mask
