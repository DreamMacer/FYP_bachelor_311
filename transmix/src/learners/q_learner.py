...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.double_q:
            ...
            target_max_qvals = th.gather(target_mac_out, 3, cur_max_actions).squeeze(3)#选择的action是由Q estimate最大值所决定的，提取的Q target由action 决定
        else:
        ...
        # assert 1==0,f"rewards: {rewards.shape}, target_max_qvals: {target_max_qvals.shape}, terminated: {terminated.shape}"
        targets = rewards + self.args.gamma * (1 - terminated) * target_max_qvals

        # Td-error
        td_error = (chosen_action_qvals - targets.detach())#将 targets 从计算图中分离，不计算梯度。这是因为目标值是固定的，不参与反向传播
